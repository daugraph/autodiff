...
class Node(object):
    ...
    def __mul__(self, other):
        """TODO: Your code here"""
        """Multi two nodes return a new node."""
        if isinstance(other, Node):
...
class MatMulOp(Op):
    ...
    def gradient(self, node, output_grad):
        ...
        """
        """TODO: Your code here"""
        mat_a_grad = matmul_op(output_grad, node.inputs[1], False, True)
        mat_b_grad = matmul_op(node.inputs[0], output_grad,  True, False)
        return [mat_a_grad, mat_b_grad]
...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        ...
        for idx, input_node in enumerate(node.inputs):
            ...
            node_to_output_grads_list[input_node].append(input_grads[idx])
